...
M_PROMPT = "a superhero protecting the world, ((anime style)), anime landscape, cel shading, 90's anime style, very detailed, 4k, movie scene, crisp, by studio ghibli, by toei animation"
M_NEG_PROMPT = "disfigured, kitsch, ugly, oversaturated, grain, low-res, Deformed, blurry, bad anatomy, disfigured, poorly drawn face, mutation, mutated, extra limb, ugly, poorly drawn hands, missing limb, blurry, floating limbs, disconnected limbs, malformed hands, blur, out of focus, long neck, long body, ugly, disgusting, poorly drawn, childish, mutilated, mangled, old, surreal"

# Using default sizes for low GPU usage
M_WIDTH = 512#get_output_width(scene)
M_HEIGHT = 512#get_output_heigth(scene)

M_SEED = -1#1312700805#961875063#
M_DEPTH_MODEL = 'control_sd15_depth [fef5e48e]' #use the correspoding reference
M_CANNY_MODEL = 'control_sd15_canny [fef5e48e]' #use the correspoding reference
...
def send_to_api(scene):
    ...
    if is_send_depth:
        print(get_asset_path(comp_output_depth_filename))
